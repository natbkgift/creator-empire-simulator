...
import { isProductionComplete, productionStatuses, taskIsUnlocked, workflowRecommendation, workflowStatuses, workflowStageRank, workflowStatusLabels } from '../domain/workflow.js';
...
export const activePipelineStatuses: ProjectStatus[] = productionStatuses.filter((status) => !['idea-backlog', 'published'].includes(status));
...
export const creditStats = (workspace: Workspace): { used: number; avgGeneration: number; avgUsable: number; avgCompletedVideo: number; waste: number; capacity: number } => {
  ...
  const regenerationWaste = entries.reduce((sum, entry) => sum + entry.regenerations * (actualCreditsUsed(entry) / Math.max(1, entry.generations + entry.regenerations)), 0);
  const avgCompletedVideo = completed.length ? completedUsed / completed.length : 95;
  return { used, avgGeneration: generations ? used / generations : 0, avgUsable: usable ? used / usable : 0, avgCompletedVideo, waste: regenerationWaste, capacity: Math.max(0, Math.floor(workspace.settings.capcutBalance / Math.max(1, avgCompletedVideo))) };
};

export const capacityStats = (workspace: Workspace): { hoursUsed: number; hoursAvailable: number; percent: number; activeChannels: number; wip: number; warning: string } => {
  ...
  const wip = workspace.projects.filter((project) => activePipelineStatuses.includes(project.status) && !isProductionComplete(project)).length;
  const warning = activeChannels > workspace.settings.activeChannelLimit
    ? 'จำนวนช่องที่ Active เกิน Capacity ที่ตั้งไว้'
    : percent > 110 ? 'เวลาที่วางแผนเกินชั่วโมงที่มี ควรลด Cadence หรือ Batch งาน'
      : wip > 8 ? 'งานระหว่างผลิตสูง ควรปิดงานก่อนเปิดหัวข้อใหม่' : 'Capacity อยู่ในระดับควบคุมได้';
...
const pendingUnlockedTasks = (workspace: Workspace): CalendarTask[] => focusedTasks(workspace)
  ...
  .toSorted((a, b) => Number(a.date > todayIso()) - Number(b.date > todayIso()) || (b.priority ?? 0) - (a.priority ?? 0) || `${a.date}${a.startTime}`.localeCompare(`${b.date}${b.startTime}`));
...
export const nextBestAction = (workspace: Workspace): { title: string; reason: string; route: string; project?: VideoProject; task?: CalendarTask; xp: number } => {
  ...
  if (active) {
    ...
    return {
      ...
      reason: task ? `Calendar-driven mission · ${task.durationMinutes} นาที · ${workflowStatusLabels[active.status]} → ${task.targetStatus ? workflowStatusLabels[task.targetStatus] : 'Next'}` : recommendation.detail,
      route: task?.missionRoute ?? recommendation.route, project: active, task, xp: recommendation.xp,
    };
  }
  const task = pendingUnlockedTasks(workspace)[0];
  if (task) { const project = projectById(workspace, task.projectId); return { title: task.title, reason: 'ภารกิจที่มี Priority สูงสุดใน Portfolio', route: task.missionRoute ?? 'calendar', project, task, xp: 30 }; }
...
export const dailyMission = (workspace: Workspace): { title: string; detail: string; progress: number; xp: number; route: string; projectId?: string; taskId?: string } => {
  const action = nextBestAction(workspace); const project = action.project;
  return { title: action.title, detail: action.task ? `${action.task.date} · ${action.task.startTime} · ${action.task.durationMinutes} นาที · ${action.reason}` : action.reason, progress: project ? projectProgress(project.status) : 0, xp: action.xp, route: action.route, projectId: project?.id, taskId: action.task?.id };
};

export const analyticsForChannel = (workspace: Workspace, channelId?: string): AnalyticsEntry[] => workspace.analytics.filter((entry) => !channelId || entry.channelId === channelId);
export const analyticsInFocus = (workspace: Workspace, projectId?: string): AnalyticsEntry[] => {
  ...
  const channel = focusedChannel(workspace); return workspace.analytics.filter((entry) => !channel || entry.channelId === channel.id);
};

export const taskWindow = (workspace: Workspace, days = 7): CalendarTask[] => {
  const start = todayIso(); const end = addDays(start, days - 1);
  return focusedTasks(workspace).filter((task) => task.date >= start && task.date <= end).sort((a,b) => `${a.date}${a.startTime}`.localeCompare(`${b.date}${b.startTime}`));
};

export const projectProgress = (status: ProjectStatus): number => {
  if (workflowStageRank(status) >= workflowStageRank('published')) return 100;
  const index = Math.max(0, productionStatuses.indexOf(status));
  return Math.max(0, Math.round((index / (productionStatuses.length - 1)) * 100));
};
...
export const channelHealth = (workspace: Workspace, channel: Channel): number => {
  ...
  const consistency = workspace.projects.filter((project) => project.channelId === channel.id && isProductionComplete(project)).length * 5;
