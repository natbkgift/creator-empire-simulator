...
export interface LanguageScore {
  ...
  evergreen: number;
  trendDependency: 'low' | 'medium' | 'high';
  factCheckDifficulty: 'low' | 'medium' | 'high';
  copyrightRisk: RiskLevel;
  aiVisualSuitability: number;
  sponsorPotential: number;
  affiliatePotential: number;
  digitalProductPotential: number;
...
export interface Idea {
  ...
  score: Record<Language, LanguageScore>;
  saved: boolean;
  isDemo?: boolean;
}
...
export interface ChannelBlueprint {
  ...
  narrationPersonality: string;
  languageStrategy: string;
  voice: string;
  shortsStrategy: string;
  longFormStrategy: string;
  plan30Days: ContentPlanItem[];
  experiment90Days: string[];
  monetizationPaths: string[];
  risks: string[];
  originalityStrategy: string;
  factCheckWorkflow: string[];
  sourcePolicy: string;
...
export interface Channel {
  ...
  health: number;
  weeklyHours: number;
  weeklyShortsTarget: number;
  monthlyLongTarget: number;
...
export interface SourceRecord {
  ...
  publisher: string;
  accessedAt: string;
  claimType: 'documented' | 'reported' | 'disputed' | 'context';
  notes: string;
}
...
export interface CreditEntry {
  ...
  balanceAfter: number;
  tool: 'full-video' | 'video-clip' | 'ai-image' | 'voiceover' | 'captions' | 'avatar' | 'other';
...
export interface CalendarTask {
  ...
  type: 'research' | 'script' | 'editing' | 'upload' | 'analytics' | 'rest' | 'other';
  projectId?: string;
...
export interface MonetizationPath {
  ...
  channelId: string;
  name: string;
  type: 'platform' | 'off-platform' | 'lead-generation';
  status: 'idea' | 'testing' | 'active' | 'paused';
  estimatedMonthlyThb: number;
  confirmedMonthlyThb: number;
  nextAction: string;
}
...
export interface SimulationInputs {
  ...
  ctrPercent: number;
  subscriberConversionPercent: number;
  productionQuality: number;
  topicRepeatability: number;
  language: Language;
  highValueAudiencePercent: number;
...
export interface SimulationScenarioResult {
  scenario: 'conservative' | 'base' | 'growth' | 'breakout';
  publishCapacity: number;
  subscribersLow: number;
  subscribersHigh: number;
  watchHoursLow: number;
  watchHoursHigh: number;
  creditsRequired: number;
  financialRequirementThb: number;
  milestoneMonthsLow: number;
  milestoneMonthsHigh: number;
  bottleneck: string;
  improvement: string;
...
export interface Settings {
  ...
  capcutBalance: number;
  soundEnabled: boolean;
  reducedMotion: boolean;
  onboardingComplete: boolean;
  workflowMode: AutomationMode;
  aiProvider: AiProvider;
  openAiModel: string;
  geminiModel: string;
  connectedAiEnabled: boolean;
