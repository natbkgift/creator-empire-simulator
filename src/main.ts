...
void initializeStore().then((workspace) => {
  ...
  if ('serviceWorker' in navigator) {
    const registerServiceWorker = (): void => { navigator.serviceWorker.register('./sw.js?v=1.4.1', { updateViaCache: 'none' }).catch((error) => console.warn('Service worker registration failed.', error)); };
