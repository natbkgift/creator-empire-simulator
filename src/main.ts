...
import { registerRenderer, requestRender } from './app/runtime.js';
...
import { rebuildWorkflowTasks, syncNextWorkflowMission } from './domain/workflow.js';
...
const render = (): void => {
  ...
  try {
    ...
    if (!workspace.settings.onboardingComplete && route !== 'onboarding') {
      navigate('onboarding');
      return;
    }
    if (workspace.settings.onboardingComplete && route === 'onboarding') {
...
app.addEventListener('submit', (event) => {
  ...
  if (formName === Forms.IDEA_FILTER) {
    ...
    navigate('ideas', {
      q: stringFrom(data, 'q'),
      category: stringFrom(data, 'category', 'all'),
      lang: current.get('lang') ?? 'en',
      format: current.get('format') ?? 'both',
    });
    ...
    navigate('prompts', {
      ...
      type: stringFrom(data, 'type', 'shorts-script'),
      lang: stringFrom(data, 'lang', 'en'),
      advanced: data.get('advanced') === '1' ? '1' : '0',
      short: data.get('short') === '1' ? '1' : '0',
    });
...
app.addEventListener('click', (event) => {
  ...
  if (action === Actions.OPEN_COMMAND) openCommandPalette();
    ...
    if (projectId) {
      ...
      showToast('สร้าง Calendar-driven workflow ใหม่แล้ว');
      navigate('mission', { project: projectId });
    }
  }
    ...
    updateWorkspace((draft) => {
      ...
      awardXp(draft, 'onboarding-complete', 60, 'consistency');
    });
    navigate('hq');
    ...
    updateWorkspace((draft) => {
      ...
      if (idea) idea.saved = !idea.saved;
    });
    showToast('อัปเดต Saved Ideas แล้ว');
  } else if (action === Actions.COPY_CAPCUT_PROMPT) {
    void copyText(document.querySelector<HTMLTextAreaElement>('#capcut-prompt')?.value ?? '').then(() => showToast('คัดลอก CapCut Prompt แล้ว'));
  } else if (action === Actions.COPY_PROMPT) {
    void copyText(document.querySelector<HTMLTextAreaElement>('#prompt-output')?.value ?? '').then(() => showToast('คัดลอก Prompt ไปใช้กับ ChatGPT ได้แล้ว'));
  ...
  else if (action === Actions.TOGGLE_TASK) {
    const taskId = target.dataset.taskId;
    if (taskId) toggleCalendarTask(taskId);
  } else if (action === Actions.RESET_SIMULATOR) {
    updateWorkspace((draft) => { draft.simulations = []; });
    showToast('Reset Simulator แล้ว');
  } else if (action === Actions.RECALCULATE_RISK) {
    const projectId = target.dataset.projectId;
    if (projectId) recalculateProjectRisk(projectId);
  } else if (action === Actions.SAVE_SETTINGS) {
    document.querySelector<HTMLFormElement>(`[data-form="${Forms.SETTINGS}"]`)?.requestSubmit();
  ...
  else if (action === Actions.REMOVE_DEMO_DATA) {
    void confirmDialog('Remove Demo Data', 'ลบ Demo Channels, Projects, Calendar, Analytics และ Credits โดยเก็บข้อมูลจริงไว้?', 'ลบ Demo').then((confirmed) => {
      if (confirmed) removeDemoData();
    });
    ...
    void confirmDialog('Reset entire workspace', 'ข้อมูลทั้งหมดจะถูกแทนที่ด้วย Demo Workspace เริ่มต้น การดำเนินการนี้ย้อนกลับไม่ได้หากไม่มี Backup', 'Reset').then((confirmed) => {
      ...
      seed.settings.onboardingComplete = true;
      replaceWorkspace(seed);
      showToast('Reset Workspace แล้ว');
      navigate('hq');
    });
  }
});
...
app.addEventListener('change', (event) => {
  ...
  if (target.dataset.change === Changes.ACTIVE_CHANNEL) {
    ...
    updateWorkspace((draft) => {
      ...
      project.riskLevel = completed >= 6 ? 'low' : completed < 3 ? 'high' : 'review';
      project.updatedAt = new Date().toISOString();
      applyProjectFocus(draft, project.id);
    });
  }
  if (target.id === 'workspace-file' && target instanceof HTMLInputElement && target.files?.[0]) {
    void importWorkspaceFile(target.files[0]);
  }
  if (target.id === 'analytics-csv' && target instanceof HTMLInputElement && target.files?.[0]) {
    void importAnalyticsCsv(target.files[0]);
  }
});
...
void initializeStore().then((workspace) => {
  subscribe(() => render());
  if (!location.hash) {
    location.hash = workspace.settings.onboardingComplete ? '#/hq' : '#/onboarding';
  } else {
    render();
  }
