...
export const renderAnalytics = (workspace: Workspace, params: URLSearchParams): string => {
  ...
  return `${pageHeader('Insights', 'Learn from published work without turning the workspace into a wall of metrics.', `<label class="btn" for="analytics-csv">Import CSV</label><input id="analytics-csv" class="file-input sr-only" type="file" accept=".csv,text/csv"/><a class="btn primary" href="${routeHref('mission', project ? { project: project.id } : undefined)}">Current Mission</a>`)}
    ...
    <div class="insights-two-column">
      <section class="panel decision-signal"><div class="row between"><h3>Decision signal</h3>${entries.some((entry) => entry.isDemo) ? dataLabel('demo') : entries.length ? dataLabel('actual') : chip('No data', 'muted')}</div><p class="decision-copy">${escapeHtml(nextInsight)}</p><div class="decision-metrics"><div><span>Avg retention</span><b>${avgRetention.toFixed(1)}%</b></div><div><span>Avg CTR</span><b>${avgCtr.toFixed(1)}%</b></div><div><span>Records</span><b>${entries.length}</b></div></div></section>
