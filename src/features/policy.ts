...
import { requiredPolicyChecks, workflowReadiness } from '../domain/workflow.js';
...
export const renderPolicy = (workspace: Workspace, params: URLSearchParams): string => {
  ...
  const derivedRisk = !project ? 'review' : mandatoryMissing.length === 0 && project.riskLevel === 'low' ? 'low' : mandatoryMissing.length >= 4 ? 'high' : 'review';
