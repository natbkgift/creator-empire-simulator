...
export const createChannelFromForm = (form: HTMLFormElement): void => {
  ...
  if (!idea) { showToast('ไม่พบ Idea ที่เลือก', 'danger'); return; }
  ...
  let suffix = 2;
  while (existingHandles.has(handle)) { handle = `${baseHandle}-${suffix}`; suffix += 1; }
  const now = new Date().toISOString();
  const channel: Channel = {
    id: uid('channel'), name: requestedName, handle, ideaId: idea.id,
    niche: language === 'en' ? idea.titleEn : idea.titleTh, language, role, health: 55,
    ...
    audienceCountries: idea.score[language].recommendedCountries, blueprint, createdAt: now,
  };
  updateWorkspace((draft) => {
    ...
    draft.monetization.push({
      id: uid('money'), channelId: channel.id, name: blueprint.monetizationPaths[0] ?? 'Primary monetization experiment',
      type: idea.categoryId === 'ai' || idea.categoryId === 'thailand' ? 'lead-generation' : 'platform', status: 'idea',
      estimatedMonthlyThb: 0, confirmedMonthlyThb: 0,
      nextAction: language === 'en' ? 'Publish a proof set before activating this path.' : 'สร้างชุดคลิปพิสูจน์ผลก่อนเปิดใช้เส้นทางนี้',
    });
  });
  const nextWorkspace = getWorkspace();
  const capacity = capacityStats(nextWorkspace);
  showToast(capacity.activeChannels > nextWorkspace.settings.activeChannelLimit ? 'สร้าง Channel แล้ว แต่จำนวนช่อง Active เกิน Capacity ที่ตั้งไว้' : 'สร้าง Channel Blueprint แล้ว', capacity.activeChannels > nextWorkspace.settings.activeChannelLimit ? 'warning' : 'success');
...
export const createProjectFromForm = (form: HTMLFormElement): void => {
  ...
  if (!channel) { showToast('ไม่พบ Channel', 'danger'); return; }
  ...
  const now = new Date().toISOString();
  const fallbackDate = addDays(todayIso(), format === 'shorts' ? 5 : 12);
  const rawPublish = stringFrom(data, 'deadline') || `${fallbackDate}T${workspace.settings.defaultPublishTime || '19:00'}`;
  const publishAt = rawPublish.includes('T') ? rawPublish : `${rawPublish}T${workspace.settings.defaultPublishTime || '19:00'}`;
  const project: VideoProject = {
    id: uid('project'), title: stringFrom(data, 'title') || channel.niche, channelId: channel.id, ideaId: channel.ideaId,
    series: channel.niche, language: channel.language,
    platforms: format === 'shorts' ? ['YouTube Shorts', 'Facebook Reels', 'TikTok'] : ['YouTube'],
    format, targetDurationSeconds: format === 'shorts' ? 55 : 480, deadline: publishAt.slice(0,10), publishAt,
    owner: 'Nat', estimatedMinutes: score?.productionMinutes ?? (format === 'shorts' ? 120 : 420), budgetThb: 0,
    creditEstimateLow: score?.estimatedCreditsLow ?? 40, creditEstimateHigh: score?.estimatedCreditsHigh ?? 120,
    actualCredits: 0, status: 'selected', growthLoopStatus: 'pending', sourceIds: [], researchSummary: '', factCheckSummary: '',
    scriptVersion: 0, script: '', hook: '', storyboard: [], assetPrompts: [], capcutBrief: '', promptVersions: [], thumbnailVersions: [],
    publicationLinks: [], lessonsLearned: '', analyticsPostmortem: '', repurposingPlan: '', workflowEvents: [], policyChecks: {},
    riskLevel: score?.copyrightRisk ?? 'review', createdAt: now, updatedAt: now,
  };
  updateWorkspace((draft) => {
  ...
  const conflicts = getWorkspace().calendarTasks.filter((task) => task.projectId === project.id && task.autoGenerated && task.conflict).length;
  showToast(conflicts ? `วางแผนวิดีโอแล้ว แต่มี ${conflicts} Capacity conflict` : 'วางแผนวิดีโอและ Production Calendar แล้ว', conflicts ? 'warning' : 'success');
  navigate('calendar', { project: project.id });
};
...
export const recordCreditEntry = (form: HTMLFormElement): void => {
  ...
  if (after > before) { showToast('Balance หลังสร้างต้องไม่มากกว่า Balance ก่อนสร้าง', 'warning'); return; }
  const entry: CreditEntry = {
    id: uid('credit'), projectId: project?.id, channelId: project?.channelId, createdAt: new Date().toISOString(),
    balanceBefore: before, balanceAfter: after, tool: stringFrom(data, 'tool', 'other') as CreditEntry['tool'],
    mode: stringFrom(data, 'mode', 'standard') as CreditEntry['mode'], model: stringFrom(data, 'model'),
    durationSeconds: Math.max(0, numberFrom(data, 'durationSeconds')), resolution: stringFrom(data, 'resolution', 'other') as CreditEntry['resolution'],
  ...
  let gateBlocker = '';
  updateWorkspace((draft) => {
    draft.credits.push(entry); draft.settings.capcutBalance = after;
    const target = draft.projects.find((item) => item.id === project?.id);
    if (target) {
      target.actualCredits += Math.max(0, before - after); target.updatedAt = new Date().toISOString();
      const previous = target.status;
      const candidate: ProjectStatus | undefined = previous === 'capcut-draft' ? 'editing' : previous === 'editing' && entry.completedVideo ? 'qa' : undefined;
      if (candidate) {
        ...
        if (readiness.ready) {
          ...
          target.status = candidate; advancedTo = candidate;
        ...
        } else { gateBlocker = readiness.blockers[0] ?? ''; applyProjectFocus(draft, target.id); }
      } else applyProjectFocus(draft, target.id);
    }
    awardXp(draft, `credit:${entry.id}`, 30, 'editing');
    if (draft.credits.length >= 5) { const achievement = draft.achievements.find((item) => item.id === 'achievement_cost_control'); if (achievement && !achievement.unlockedAt) achievement.unlockedAt = new Date().toISOString(); }
  });
...
export const createCalendarTask = (form: HTMLFormElement): void => {
  ...
  const base: Omit<CalendarTask, 'id' | 'date'> = {
    title: stringFrom(data, 'title'), startTime: stringFrom(data, 'startTime', '09:00'),
    durationMinutes: Math.max(5, numberFrom(data, 'durationMinutes', 30)), type: stringFrom(data, 'type', 'other') as CalendarTask['type'],
    projectId: stringFrom(data, 'projectId') || undefined, recurring, completed: false, templateKind: 'manual',
  };
  if (!base.title) { showToast('กรุณาใส่ชื่อภารกิจ', 'warning'); return; }
  const date = stringFrom(data, 'date', todayIso());
  const dates = recurring === 'daily' ? Array.from({ length: 7 }, (_, index) => addDays(date, index)) : recurring === 'weekly' ? Array.from({ length: 4 }, (_, index) => addDays(date, index * 7)) : [date];
  updateWorkspace((draft) => {
    dates.forEach((taskDate, index) => draft.calendarTasks.push({ ...base, id: uid('task'), date: taskDate, recurring: index === 0 ? recurring : 'none' }));
...
export const toggleCalendarTask = (taskId: string): void => {
  ...
  if (task.autoGenerated && project) {
    ...
    completeMission(project.id, task.id); return;
  }
  updateWorkspace((draft) => {
    const target = draft.calendarTasks.find((item) => item.id === taskId); if (!target) return;
    target.completed = !target.completed; target.completedAt = target.completed ? new Date().toISOString() : undefined;
...
export const addAnalyticsEntry = (form: HTMLFormElement): void => {
    ...
    if (target) {
      ...
      if (target.status === 'published') {
        ...
        if (readiness.ready) {
          ...
          if (task) { task.completed = true; task.completedAt = new Date().toISOString(); }
          target.status = 'analytics-review'; target.growthLoopStatus = 'analytics'; target.updatedAt = new Date().toISOString(); advanced = true;
          recordWorkflowEvent(target, { type: 'stage-advanced', note: 'Growth loop started with actual analytics.', fromStatus: 'published', toStatus: 'analytics-review', taskId: task?.id });
          const next = syncNextWorkflowMission(draft, target.id); nextTaskId = next?.id; applyProjectFocus(draft, target.id, next?.id);
        }
      } else applyProjectFocus(draft, target.id);
    }
  ...
  showToast(advanced ? 'บันทึก Analytics แล้ว · Production ยังคง Complete และเริ่ม Growth Loop' : 'บันทึก Actual analytics แล้ว');
...
export const addMonetizationPath = (form: HTMLFormElement): void => {
  ...
  const path: MonetizationPath = {
    id: uid('money'), channelId: stringFrom(data, 'channelId'), name: stringFrom(data, 'name'),
    type: stringFrom(data, 'type', 'off-platform') as MonetizationPath['type'], status: stringFrom(data, 'status', 'idea') as MonetizationPath['status'],
    estimatedMonthlyThb: Math.max(0, numberFrom(data, 'estimatedMonthlyThb')), confirmedMonthlyThb: Math.max(0, numberFrom(data, 'confirmedMonthlyThb')), nextAction: stringFrom(data, 'nextAction'),
  };
  if (!path.channelId || !path.name || !path.nextAction) { showToast('กรอก Channel, Name และ Next action ให้ครบ', 'warning'); return; }
  updateWorkspace((draft) => { draft.monetization.push(path); awardXp(draft, `money:${path.id}`, 25, 'monetization'); });
...
export const recalculateProjectRisk = (projectId: string): void => {
  ...
  showToast('อัปเดต Policy risk จาก Mandatory checks แล้ว');
};
...
export const runSimulatorFromForm = (form: HTMLFormElement): void => {
  ...
  const inputs: SimulationInputs = {
    ...
    averageViews: Math.max(0, numberFrom(data, 'averageViews', defaultSimulationInputs.averageViews)), retentionPercent: Math.max(0, numberFrom(data, 'retentionPercent', defaultSimulationInputs.retentionPercent)),
    ctrPercent: Math.max(0, numberFrom(data, 'ctrPercent', defaultSimulationInputs.ctrPercent)), subscriberConversionPercent: Math.max(0, numberFrom(data, 'subscriberConversionPercent', defaultSimulationInputs.subscriberConversionPercent)),
    productionQuality: Math.min(100, Math.max(0, numberFrom(data, 'productionQuality', defaultSimulationInputs.productionQuality))), topicRepeatability: Math.min(100, Math.max(0, numberFrom(data, 'topicRepeatability', defaultSimulationInputs.topicRepeatability))),
    language: (stringFrom(data, 'language') === 'th' ? 'th' : 'en') as Language, highValueAudiencePercent: Math.min(100, Math.max(0, numberFrom(data, 'highValueAudiencePercent', defaultSimulationInputs.highValueAudiencePercent))),
    monthlyBudgetThb: Math.max(0, numberFrom(data, 'monthlyBudgetThb', defaultSimulationInputs.monthlyBudgetThb)), availableCredits: Math.max(0, numberFrom(data, 'availableCredits', getWorkspace().settings.capcutBalance)),
    hoursPerWeek: Math.max(1, numberFrom(data, 'hoursPerWeek', getWorkspace().settings.weeklyHoursAvailable)), consistencyPercent: Math.min(100, Math.max(0, numberFrom(data, 'consistencyPercent', defaultSimulationInputs.consistencyPercent))),
    seed: Math.trunc(numberFrom(data, 'seed', defaultSimulationInputs.seed)),
  };
  const simulation = runSimulation(inputs);
  updateWorkspace((draft) => { draft.simulations.push(simulation); if (draft.simulations.length > 20) draft.simulations = draft.simulations.slice(-20); awardXp(draft, `simulation:${simulation.id}`, 20, 'analytics'); });
